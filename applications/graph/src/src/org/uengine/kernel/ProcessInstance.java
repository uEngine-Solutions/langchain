		...
		ActivityForLoop forLoop = new ActivityForLoop() {
			public void logic(Activity act) {
				try {
					...
					String instanceId = instancehere.getInstanceId();
					String status = act.getStatus(instancehere);
					
					if (act instanceof SubProcessActivity && (status.equals(Activity.STATUS_COMPLETED) || status.equals(Activity.STATUS_RUNNING))) {
						SubProcessActivity spAct = (SubProcessActivity) act;
						...
						if ( filter == null
								|| filter.indexOf(status) > -1 
								|| (Activity.STATUS_RUNNING.equals(filter) && (Activity.STATUS_RUNNING.equals(status) || Activity.STATUS_TIMEOUT.equals(status))) ) {
							runningActivities.add(new ActivityInstanceContext(act, instancehere));
						}
					}
					
//					} else if (!(act instanceof ComplexActivity) && (filter == null || filter.indexOf(status) > -1)) {
//						runningActivities.add(new ActivityInstanceContext(act, instancehere));
//					}
				} catch (Exception e) {
					throw new RuntimeException(e);
