...
public class HumanActivityTest extends TestCase{
	...
	public void testMultipleActor(){
		try{
			...
			payload.setExtendedValues(new KeyedParameter[]{
				new KeyedParameter(HumanActivity.PAYLOADKEY_TASKID, "0")
			});
			process.fireMessage(process.humanActivity.getMessage(), instance, payload);

			payload.setExtendedValues(new KeyedParameter[]{
				new KeyedParameter(HumanActivity.PAYLOADKEY_TASKID, "2")
			});
			process.fireMessage(process.humanActivity.getMessage(), instance, payload);
			...
			payload.setExtendedValues(new KeyedParameter[]{
				new KeyedParameter(HumanActivity.PAYLOADKEY_TASKID, "1")
			});
			process.fireMessage(process.humanActivity.getMessage(), instance, payload);
