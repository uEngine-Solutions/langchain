 ...
 */
public class PostgresDAOFactory extends OracleDAOFactory{
				...
				public int call() throws Exception{					
					...
					if(var_taskId==null){
						IDAO valueDAO = ConnectiveDAO.createDAOImpl(
							getConnectionFactory(),
							"SELECT nextval('SEQ_BPM_WORKITEM') as TASKID",
							IDAO.class
						);
						valueDAO.select();
						valueDAO.next();
...
*/
	public KeyGeneratorDAO createKeyGenerator(String forWhat, Map options) throws Exception {
		return (KeyGeneratorDAO)create(
			...
			//select nextval('SEQ_BPM_WORKITEM') as TASKID;
			"SELECT nextval('SEQ_BPM_"+forWhat+"') AS keyNumber"
		);
	}
	...
	public String getSequenceSql(String seqName) throws Exception {
		...
		return " nextval('SEQ_BPM_" + seqName + "') "; 
	}
	...
	@Override
	public String getSystemDateFunction() throws Exception {
